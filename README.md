...
- UI 언어: 기기 언어가 한국어면 한국어, 그 외 영어 (`src/i18n.ts`).
- 저장: 앱은 "RatioShot" 앨범, 모바일 웹은 공유 시트, 그 외는 다운로드.
...
**서명.** 키스토어는 `~/ratioshot-release.jks`(alias `multishot`), 비밀번호는 macOS 키체인
(`security find-generic-password -a ratioshot -s ratioshot-keystore -w`). **이 파일을 잃어버리면 앱을 다시는
...
`store/out/`: Play 아이콘 512, 피처 그래픽 1024×500, 스크린샷 4장(1170×2532), 등록정보 문구(영/한).
...
`main`에 푸시하면 GitHub Pages가 `dist/`를 배포한다: https://jeongwookkim.github.io/ratioshot/
- 개인정보처리방침: https://jeongwookkim.github.io/ratioshot/privacy.html (Play 등록정보에 넣는 URL)
